...
public:
    enum class ParamId { Volume = 0, Base, Tension, Speed, Phase, Count };
    static constexpr int kNumWheels = static_cast<int>(ParamId::Count);

    using ParamChangeFn  = std::function<void(ParamId, float)>;
    using ShapeChangeFn  = std::function<void(int)>;
    using SyncChangeFn   = std::function<void(bool)>;   // true = Free

    VisageMainView() {
        const char* labels[kNumWheels] = { "VOLUME", "BASE", "TENSION", "SPEED", "PHASE" };
        const Type  types [kNumWheels] = { Type::Bipolar, Type::Percent, Type::Bipolar, Type::Speed, Type::Percent };
        const float defs  [kNumWheels] = { 0.75f, 0.5f, 0.5f, 0.666f, 0.0f };
        for (int i = 0; i < kNumWheels; ++i) {
            wheels_[i].label = labels[i]; wheels_[i].type = types[i]; wheels_[i].value01 = defs[i];
        }
    }
    ...
    void setSyncChangeCallback(SyncChangeFn fn)   { on_sync_ = std::move(fn); }
    void setWheel(ParamId id, float v01) { setWheelValue(static_cast<int>(id), v01, false); }
    void setShape(int s) { shape_ = std::clamp(s, 0, 4); redraw(); }
    void setFree(bool free) { free_ = free; redraw(); }
    void setSpeedLabel(std::string s) { speed_label_ = std::move(s); redraw(); }
    ...
    void draw(visage::Canvas& canvas) override {
        ...
        drawSyncToggle(canvas);
        drawShapeSelector(canvas);
    }

    void mouseDown(const visage::MouseEvent& e) override {
        ensureLayout();
        const auto p = e.windowPosition();   // host fills window_position; root frame at (0,0)
        for (int s = 0; s < 5; ++s)
            if (hit(seg_[s], p.x, p.y)) { shape_ = s; if (on_shape_) on_shape_(s); redraw(); return; }
        for (int m = 0; m < 2; ++m)
            if (hit(sync_[m], p.x, p.y)) { free_ = (m==1); if (on_sync_) on_sync_(free_); redraw(); return; }
        active_ = hitWheel(p.x, p.y);
        if (active_ >= 0) { drag_y_ = p.y; drag_v_ = wheels_[active_].value01; wheels_[active_].dragging = true; }
    }
...
private:
    enum class Type { Percent, Bipolar, Speed };
    ...
    void layout() {
        ...
        const float r=26.0f, col3=panel_.w/3.0f;
        const float row1=panel_.y+44.0f, row2=row1+80.0f;
        for (int i=0;i<3;++i){ wheels_[i].cx=panel_.x+col3*(i+0.5f); wheels_[i].cy=row1; wheels_[i].r=r; }
        wheels_[3].cx=panel_.x+col3*0.5f; wheels_[3].cy=row2; wheels_[3].r=r;   // Speed
        wheels_[4].cx=panel_.x+col3*1.5f; wheels_[4].cy=row2; wheels_[4].r=r;   // Phase
        // sync toggle: right third, two stacked buttons
        const float sx=panel_.x+col3*2.0f+10.0f, sw=col3-24.0f, sh=22.0f, gap=6.0f, sy=row2-26.0f;
        sync_[0]={ sx, sy, sw, sh };            // SYNC
        sync_[1]={ sx, sy+sh+gap, sw, sh };     // FREE
        // shape selector: 5 segments across the bottom of the panel
        const float shy=panel_.y+panel_.h-30.0f, shw=(panel_.w-16.0f)/5.0f;
        for (int s=0;s<5;++s) seg_[s]={ panel_.x+8.0f+s*shw, shy, shw-3.0f, 22.0f };
    }
    ...
    void drawScope(visage::Canvas& c) {
        ...
        c.setColor(kGreen);
        const int steps=112; float lx=s.x, ly=s.y+s.h*0.5f;
    ...
    void drawWheel(visage::Canvas& c, const Wheel& k) {
        ...
        const float startV=(k.type==Type::Bipolar)?0.5f:0.0f, rr=k.r*0.9f;
        ...
        for (int i=1;i<=n;++i){ float aa=a0+(aEnd-a0)*(float)i/n; float x=k.cx+std::cos(aa)*rr,y=k.cy+std::sin(aa)*rr; c.segment(px,py,x,y,2.5f,true); px=x; py=y; }
        const float a=ang(k.value01), len=k.r*0.72f;
    ...
    void drawSyncToggle(visage::Canvas& c) {
        const char* nm[2]={"SYNC","FREE"};
        if (fonts_ready_){ c.setColor(kDim); c.text("MODE", value_font_, visage::Font::kCenter, sync_[0].x, sync_[0].y-15, sync_[0].w, 12);}
        for (int m=0;m<2;++m){ const Rect& r=sync_[m]; bool on=((m==1)==free_);
            ...
            if (fonts_ready_){ c.setColor(on?kBg:kDim); c.text(nm[m], value_font_, visage::Font::kCenter, r.x, r.y+4, r.w, 14);} }
    }

    void drawShapeSelector(visage::Canvas& c) {
        const char* nm[5]={"Sin","Tri","Sqr","Saw","Rnd"};
        for (int s=0;s<5;++s){ const Rect& r=seg_[s]; bool on=(s==shape_);
            c.setColor(on?kGreen:kWheel); c.rectangle(r.x,r.y,r.w,r.h); c.fill(r.x,r.y,r.w,r.h);
            c.setColor(kBorder); c.rectangleBorder(r.x,r.y,r.w,r.h,1.0f);
    ...
    std::string formatValue(const Wheel& k) const {
        ...
        switch (k.type) {
            ...
            case Type::Speed:
                if (!speed_label_.empty()) return speed_label_;
                { static const char* s[10]={"1/2","1","2","3","4","8","16","32","64","128"};
                  int i=std::clamp((int)std::lround(k.value01*9.0f),0,9); std::snprintf(b,sizeof(b),"%s st",s[i]); }
                break;
        }
        return b;
    }
    ...
    int hitWheel(float x, float y) const {
        for (int i=0;i<kNumWheels;++i){ float dx=x-wheels_[i].cx, dy=y-wheels_[i].cy; if (dx*dx+dy*dy<=wheels_[i].r*wheels_[i].r) return i; }
        return -1;
    }
    ...
    Rect title_, scope_, panel_, seg_[5], sync_[2];
    int shape_=0; bool free_=false;
    std::string speed_label_;
    float meter_lfo_=0.5f, meter_gain_=1.0f;
    int active_=-1; float drag_y_=0, drag_v_=0;
    float laidW_=-1, laidH_=-1;
    ParamChangeFn on_param_; ShapeChangeFn on_shape_; SyncChangeFn on_sync_;
};
