...
VAZAutopanAudioProcessorEditor::VAZAutopanAudioProcessorEditor (VAZAutopanAudioProcessor& p)
    : AudioProcessorEditor (&p), audioProcessor (p)
{
    ...
    leftAtt  = attach (ParameterIDs::left_limit,  leftRelay);
    rightAtt = attach (ParameterIDs::right_limit, rightRelay);
...
void VAZAutopanAudioProcessorEditor::paint (juce::Graphics& g) { g.fillAll (juce::Colour (0xff2a0d0d)); }

void VAZAutopanAudioProcessorEditor::resized()
{
    if (webView != nullptr) webView->setBounds (getLocalBounds());
}

const char* VAZAutopanAudioProcessorEditor::getMimeForExtension (const juce::String& extension)
{
...
VAZAutopanAudioProcessorEditor::getResource (const juce::String& url)
{
