...
| Pulse-width map | `FUN_004de930` @0x4de930 (`param[0xa4]=b<<16`) + pulse edge `vaz_big.c`:213/222 | `OscBlock::pulseWidth` | ✅ **VERIFICERET (2026-07-11)** | duty = **WaveShape/256** (LINEAR, exact square @ byte 128 = shape 0.5); traced setter + band-limited edge (`edge=b<<24` vs 32-bit phase). Oracle `osc_pulse_pwmap` **BIT-EXACT** (duty map). Replaced the asserted `0.05+0.9·shape` compression (was ~5% off at extremes). *Per-sample* bit-exactness still needs VAZ's BLEP pulse (`006dd2c0/006de2c0`) — separate M3 item. |
