...
| 🐞 **BUG FOUND (2026-07-15) — mixer-source enum OFF BY ONE** | Real VAZ (decoded from a user Osc3-vs-RingMod preset pair; only 2 bytes differ, `mix3src` @1432): **`0` = Oscillator 3, `1` = Ring Modulator** | Clone list `{"Noise","Oscillator 3","Ring Modulator",…}` → thinks **1 = Osc3**, and DSP gates on `mix3Src==1` (`SynthVoice.h`:283) | 🔴 **REAL BUG (not yet fixed)** | A real-VAZ Osc3 patch (`mix3src=0`) loads into the clone as **"Noise"** and Osc3 never sounds; every ch3 source is shifted by one. Contradicts the old RE note "audio keys off mix3Src==1". **Separate fix — own task.** |
