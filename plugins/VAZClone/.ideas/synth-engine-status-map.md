...
| Waveform generation (saw = algorithmic; **pulse = BIT-EXACT BLEP**, see below) | `vaz_big.c`:176-320 branches | `Synth.h` osc switch | VERIFICERET (read); pulse now oracle-bit-exact | address citation + `osc_pulse_blep` |
| Sample-osc interp (4-pt cubic) | `vaz_big.c`:979-991 | `Synth.h`:108 | VERIFICERET | line-by-line |
| **Pulse oscillator (band-limited)** | `vaz_big.c`:206-241 (BLEP difference-of-ramps) + `FUN_004de930` | **`VAZPulseOsc.h`** + `VAZPulseTables.h` | ✅ **BIT-EXACT (2026-07-11)** | **Full BLEP engine ported.** Difference of two band-limited ramps (edges @ phase 0 and the duty position `iVar9`), each corrected near its edge by the RPM-dumped step tables `kStepRise`=`DAT_006dd2c0` (`trunc(-2^30/(iv8+1))`) / `kStepFall`=`DAT_006de2c0`, indexed by the freq-dependent BLEP width `iv8`. Pure `uint32` phase accumulator (`OscBlock::phaseU`). Oracle `osc_pulse_blep` **BIT-EXACT** (sample-exact vs independent transcription; pre-port harmonic dev was up to 0.12 on narrow/wide pulses + high notes). Duty map `osc_pulse_pwmap` still BIT-EXACT; the `b/256` map + VAZ's edge clamp now live inside the recurrence (`pw=b<<16`, `iVar9` clamp). Replaces the naive difference-of-saws. |
