...
| **Saw** waveform gen (band-limited ramp) | 🔴 **NOT BLEP — corrected 2026-07-17.** `DAT_006dd2c0/006de2c0` are **RECIPROCAL SLOPE tables**, proven vs the raw dump over all 1024 entries: `dd2c0[i] = −2³⁰/(i+1)`, `de2c0[i] = 2³⁰/(2047−i)` — **plain C INTEGER DIVISION (truncates toward zero), NOT round()/fistp** (an earlier note said `round(...)`: wrong. `2³⁰/2046 = 524800.5005` → dump has **524800**; any nearest-rule gives 524801. Caught by asserting the dump anchors in the oracle primitive — keep that check). VAZ's saw (audio branch **`0x4DCEF5`**, selector `[+0x1ac]`==0) is a **2-segment piecewise-linear "slope-limited saw"**: `idx = max(freqIdx, min(\|shape\|,0x7fffff))>>13`, `thr=(idx+1)*0x2000−0x800000`, `out=(saw∓0x800000)*TBL[idx]>>19 + 0x800000`, where `freqIdx=max((inc>>8)+(inc>>11)−0xe000,0)`. `idx` = **falling-edge width** (0 → sharp saw; 1020 → ~49.9% = triangle) and is VAZ's **entire** anti-aliasing (saw morphs toward tri as pitch rises, from ~134 Hz up). Phase runs **downward** (`sub eax,inc`); hard-sync hook `call 0x4dfbf8` when `[+0x1ec]`. | `Synth.h` case 0 — **additive** mip-mapped wavetables + `saw*(1−s)+tri*s` crossfade | ⚠ **DEVIATION (mid-shape) — reported 2026-07-17, NOT yet fixed; staged behind real-VAZ render verification** | The old "spectra match ±0.03 at C3" test only probed **default shape** and compares **magnitude spectra** — structurally blind to polarity and to the shape sweep. Endpoints agree (s=0 sharp saw; s=1 → idx=1020 ≈ triangle) but **mid-shape genuinely differs**: VAZ s=0.5 = clean 25%-edge ramp (full ±FS, no DC); the crossfade gives `3t−1 / 1−t` = **DC-offset + amplitude-reduced**. The reverted `VAZPulseOsc.h` "BLEP port" (d473dad) was built on this **false BLEP premise** — do not re-attempt it as BLEP. **Fix gated on a real-VAZ render capture** (the one route that has ever held; see the BLEP revert). |
