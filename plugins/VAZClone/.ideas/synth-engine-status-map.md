...
| 6 | **Multi-saw detune → cents** | 1.1 | ✅ **VERIFIED-exact 2026-07-17 — mechanism DISASSEMBLED (was "ENTANGLED, don't guess" — that verdict was WRONG)** | common osc | **NOT entangled** — the sub-osc loop is explicit at `0x4DCAA5..0x4DCB2B`. Audio-osc selector is **`[+0x1ac]`** (`==2` = Multi-Saw), *not* `[+0x84]` (that's the mod-LFO). Exact law: `ecx = (shapeByte<<15) + shapeMod` (`0x4DCA6F`); `d = clamp(ecx,0,0x7fff00)>>18` = **`b>>3` ∈ [0,31]**; 4 phases (`[esi+0xc4/0xc8/0xcc/0xd0]`) read increments from pitch table `DAT_005445e0` at **base + {−1.5, −0.5, +0.5, +1.5}·d** steps; table = **128 steps/semitone → 0.78125 cents/step** (`note_idx_006dd0c0[n]=n·128`; measured ratio → 1536 steps/oct). Max (b=255, d=31) = **0/24.22/48.44/72.66 c**. Spacing pattern (outer/inner = 3.0) **matches the clone exactly**. ⚠ **Two real (minor) deviations, NOT fixed — reported not guessed:** (1) scale — VAZ `outer = 1.5·(b>>3)·0.78125 ≈ 0.1465·b` vs clone `36.25·b/255 ≈ 0.1422·b`: equal at the endpoints (an FFT-at-Max fit) but VAZ is **~3% wider mid-knob, worst +0.97c @ b≈224**; (2) VAZ quantises to **32 distinct values** (`b>>3`), clone is continuous. Both sub-cent → deferred as a low-risk param-map fix, not a waveform change. |
...
| 9 | ~~**Osc3 audio-rate footage pitch**~~ | 1.1 | ✅ **VERIFICERET 2026-07-15 — MEASURED vs real VAZ audio** (8/8 pts within 3%, worst 2.1%) | uncommon | **CLOSED by audio-render extraction.** Clone's `2^((b−144)/48)` **CONFIRMED** in absolute pitch over 4 octaves incl. NON-octave points (rules out an octave-stepped law); rate-curve refuted; `+0x94` does NOT drive pitch → clone's `lfo1rate` source is correct. Oracle `osc3_footage_pitch` = VERIFIED. |
...
| **Saw** waveform gen (band-limited ramp) | 🔴 **NOT BLEP — corrected 2026-07-17.** `DAT_006dd2c0/006de2c0` are **RECIPROCAL SLOPE tables**, proven vs the raw dump over all 1024 entries: `dd2c0[i] = −2³⁰/(i+1)`, `de2c0[i] = 2³⁰/(2047−i)` — **plain C INTEGER DIVISION (truncates toward zero), NOT round()/fistp** (an earlier note said `round(...)`: wrong. `2³⁰/2046 = 524800.5005` → dump has **524800**; any nearest-rule gives 524801. Caught by asserting the dump anchors in the oracle primitive — keep that check). VAZ's saw (audio branch **`0x4DCEF5`**, selector `[+0x1ac]`==0) is a **2-segment piecewise-linear "slope-limited saw"**: `idx = max(freqIdx, min(\|shape\|,0x7fffff))>>13`, `thr=(idx+1)*0x2000−0x800000`, `out=(saw∓0x800000)*TBL[idx]>>19 + 0x800000`, where `freqIdx=max((inc>>8)+(inc>>11)−0xe000,0)`. `idx` = **falling-edge width** (0 → sharp saw; 1020 → ~49.9% = triangle) and is VAZ's **entire** anti-aliasing (saw morphs toward tri as pitch rises, from ~134 Hz up). Phase runs **downward** (`sub eax,inc`); hard-sync hook `call 0x4dfbf8` when `[+0x1ec]`. | `Synth.h` case 0 — **additive** mip-mapped wavetables + `saw*(1−s)+tri*s` crossfade | 🔒 **PORT BLOCKED 2026-07-17 — the 2-segment model is NECESSARY BUT NOT SUFFICIENT (proven by real-VAZ capture)** | The old "spectra match ±0.03 at C3" test only probed **default shape** and compares **magnitude spectra** — structurally blind to polarity and to the shape sweep. Endpoints agree (s=0 sharp saw; s=1 → idx=1020 ≈ triangle) but **mid-shape genuinely differs**: VAZ s=0.5 = clean 25%-edge ramp (full ±FS, no DC); the crossfade gives `3t−1 / 1−t` = **DC-offset + amplitude-reduced**. The reverted `VAZPulseOsc.h` "BLEP port" (d473dad) was built on this **false BLEP premise** — do not re-attempt it as BLEP. **AA CAPTURE RESULT (2026-07-17, `tools/osc_wave_probe.py`, File→Capture of the user's `Vaz Saw.v2p` — NOT loopback):** the model predicts a frequency-tracking edge of `(9/2048)·256 ≈ **1.125 samples** at ANY pitch, which suppresses the first fold by only `sinc(π·f·T/sr)` ≈ **−6 dB**. Measured strongest inharmonic vs that prediction: **C6 (1046.5 Hz) −45.9 dB vs model ≈−27 dB; C7 (2093 Hz) −42.1 dB vs model ≈−21 dB** ⇒ **real VAZ is 15–20 dB CLEANER than the 2-segment ramp can be.** (C3 is inconclusive by construction: a raw ramp's fold-down there is ≈−44 dB, at the noise floor — it measured −39.3 dB, one peak. The C6/C7 folds are genuine: peaks spaced by exactly f0 with their offset from the harmonic grid *shrinking* with frequency = a descending aliased series against the ascending harmonics.) The transcription is otherwise CONFIRMED (b=255 → idx=1020 matches the clone's triangle to 0.004 FS; the h4 spectral null at b=130 is the fingerprint of the predicted 25.4% edge) — so the model is **necessary but not sufficient: something else band-limits the saw.** 🔒 **DO NOT PORT the raw ramp** — that is exactly the BLEP mistake (d473dad), caught pre-port this time. **The clone's current band-limited saw is CLOSER to VAZ than the model would be → leave it alone.** Unblocking requires finding the taming stage. ❌ **Lead 1 `[+0x234]` ELIMINATED 2026-07-17** — it is the **mod-LFO's** band-limit toggle, NOT the audio osc's. All three refs sit inside the LFO dispatch (`0x4DC12C`/`0x4DC213`/`0x4DC36D`, the `[+0x84]` selector region); the audio osc's branches (`[+0x1ac]` @ `0x4DCA30`/`0x4DCA92`/`0x4DCE39`) **never consult it** — the audio saw reaches the reciprocal tables unconditionally. It was mis-ranked as lead 1 by the same **offset-overloading** confusion that caused the earlier LFO/osc mix-up (`+0x1ac` = waveform selector on the param struct but a DC-blocker accumulator on the voice struct; `+0x234` is LFO-only). ⚠ *Method note:* the first scan disassembled from `0x4d0000` — not a function entry — which **desynced x86 decoding and reported 0 refs = a confident FALSE NEGATIVE**; only re-running from the real entry `0x4dbddc` found the three. Same lesson as the flanger scan-range error: **a non-match from a misaligned/bounded disasm scan is not evidence of absence.** → **Lead 2 = OVERSAMPLING is now the front-runner** and fits unusually well: a 2× internal rate supplies ~the missing 15–20 dB, explains why VAZ can afford so crude an edge (at 2× it spans ~2.25 output samples and the first fold lands an octave up where decimation kills it), and explains why the edge is *frequency-tracking* at all — that only makes sense as a cheap pre-decimation shaper. ❌ **Lead 2 OVERSAMPLING ELIMINATED 2026-07-17 — structurally, no grep.** The phase-increment table states the internal rate directly: `inc = f/sr_internal·2^32`, and `note_idx[n]=n·128` indexes it. `curve_0x5445e0[n·128]` implies the SAME rate across six octaves (C2..C7 — a constant ratio, so a scale factor, not a tuning error), and **no reading of it is 88200**. Normalisation pinned from the dumping code itself (`dump_vaz_tables.py:82`: `f = [v/2**31 ...]` → the dump divides by **2^31**, not 2^32), so `raw = 0.00592·2^31 = 12,713,103` ≈ `f/44100·2^32 = 12,738,563` ✓ while `f/88200·2^32 = 6,369,281` is off by 2×. **Internal rate = 44100 = 1×.** (An oversampled render must advance phase at 88200; this table cannot express that. NB a literal `88200` grep was a false lead: the hit at `0x4DF0AF` was the ModRM byte `0x88` + displacement `0x158` of `mov [eax+0x158], ecx` straddling an instruction boundary; the real 88200s at `0x506064`/`0x52B39C` sit beside 22050 = a supported-device-rate list.) ⚠ minor open: the match is 0.2% off (~3.5 cents) — either `note_idx` isn't exactly `n·128` or the tuning reference differs; irrelevant to 1×-vs-2× but don't lean on it for pitch work. ❌ ✅ **FOUNDATION-CHECK 2026-07-17 — both load-bearing assumptions VERIFIED, gap survives.** (1) The increment table is a PURE exponential: `curve[i] = curve[0]·2^(i/1536)` exactly (0.00037→0.00074→0.00592 at i=0/1536/6144) ⇒ **1536 steps/oct = 128/semitone CONFIRMED**. (2) The 0.2% anomaly is a **CONSTANT −3.67-cent offset at EVERY note** (C3/C4/A4/C6 identical to 2 dp; a scale error would grow with frequency) ⇒ a tuning/index-origin offset, implied A4 = **439.07 Hz**. ⚠ **that contradicts this map's own claim of "note→pitch A440 (RPM-dumped `DAT_006dd0c0`)" — isolated OPEN question, unresolved, do not lean on either claim for pitch work.** (3) **The 1.125-sample edge is SCALE-INDEPENDENT** — the increment cancels: `edge_frac ≈ inc·9/2^35`, `cycle_samples = 2^32/inc` (structural: the phase accumulator is 32-bit), so `edge_samples = 9/8 = 1.125` regardless of the dump normalisation, the tuning offset, or any `f/sr·2^32` assumption. (4) **Predictions CORRECTED** — the earlier −27/−21 dB were the *raw* harmonic amplitudes; they under-counted the edge's own −4.6 dB. Correct: **C6 predict −31.5 dB vs measured −46.0 (gap −14.5 dB); C7 predict −25.4 dB vs measured −42.5 (gap −17.1 dB)**. The gap SHRINKS from ~20 dB to 14.5–17.1 dB but does NOT close ⇒ the model is genuinely insufficient and this row's verdict STANDS. ⚠ **caveat: both folds land at the SAME 21077 Hz** (h22 of C6 and h11 of C7) — the two measurements are NOT independent, and the over-suppression may be specific to ~21 kHz (an output/reconstruction stage or the capture chain near Nyquist) rather than the oscillator. Test folds at other frequencies before hunting a 5th mechanism. ❌ **Lead 3 POST-OSC FILTER ELIMINATED 2026-07-17** — verified by TRANSFER FUNCTION, not by the dump's filename. Solving the `[esi+0x1ac]/[0x1b0]` 64-bit accumulator: `S_new = S_old + hi·COEF + (avg[n]−avg[n−1])·2^32` ⇒ `y[n] = R·y[n−1] + (avg[n]−avg[n−1])`, `R = 1 + DAT_006df6c4/2^32` ⇒ **H(z) = (1−z⁻¹)/(1−R·z⁻¹)** = a genuine first-order **HIGH-PASS** (the DC-blocker label was right) — it removes DC, NOT high-frequency aliasing, so it cannot supply the gap. Its input `avg = ([0x170]+[0x1a4])/2` looked like a FIR low-pass (null at Nyquist ⇒ ~−23 dB at C7's first fold = suspiciously exactly the missing amount) **but is NOT**: `[0x1a4]` is written at `0x4DD560` from an accumulator gated on `[+0x23c]` = the mixer's **"Post" routing flag** (visible in VAZ's own GUI), while `[0x170]` is the **filter-chain output** (written across `0x4DDB63..0x4DE1E9`) ⇒ two DIFFERENT signals summed with a /2 for headroom = a **bus sum, not x[n]/x[n−1]**. ❌ **Lead 4 (my own test) ELIMINATED 2026-07-17** — hypothesis was that `osc_wave_probe.py` rendered through the voice filter (unlike `osc3_footage_probe.py`, which isolated with `cutoff=255,reso=0,o2level=0`). **Refuted: the user's `Vaz Saw.v2p` was ALREADY isolated** (traced: ver=203, cutoff=255, reso=0, o1level=255, o2level=0 — the filter was never in the path). Re-render with explicit isolation reproduces the floor within 0.4 dB: **C6 −46.0 dB (was −45.9), C7 −42.5 dB (was −42.1)** ⇒ the measurement was always clean and **this row's verdict STANDS**. ⚠ the probe's own verdict string printed "ALIASES → model was RIGHT" at C7 — a THRESHOLD ARTIFACT (it compared −42.5 dB against a crude −45 dB absolute cutoff instead of against the model's −21 dB prediction; −42.5 is still 21 dB BELOW the model). Read the numbers, not the verdict line. **All four leads are now dead and the taming mechanism is genuinely UNEXPLAINED.** Remaining candidates, unranked: (5) the freqIdx/edge derivation itself may be wrong — re-derive `(inc>>8)+(inc>>11)−0xe000` and the `>>13` from the bytes rather than from the earlier reading; (6) an interpolation/decimation step between the osc and `[0x170]`; (7) the increment table `DAT_005445e0` may not be `f/sr·2^32` at all (its 0.2%/3.5-cent anomaly is still unexplained and would shift every frequency-dependent conclusion). → **Superseded lead 3 note:** hypothesis: a post-osc filter stage in the voice path.** Verify the `[esi+0x1ac]/[0x1b0]` 64-bit accumulator with `DAT_006df6c4` by its actual TRANSFER FUNCTION, not by its coefficient's name in the dump (it was classed a DC-blocker from the name alone) — a one-pole in that position, between the osc sum and mixer/amp, is exactly the shape that would supply the missing 15–20 dB. **Acceptance test for any port: model + decimation must reproduce the capture (−45.9 dB @C6, −42.1 dB @C7), not the raw model's −27/−21.** |
...
| Multi-saw detune → cents | **DISASSEMBLED `0x4DCAA5..0x4DCB2B`**: `d=(shape<<15)>>18=b>>3` ∈[0,31]; 4 saws at base+{−1.5,−0.5,+0.5,+1.5}·d in `DAT_005445e0` (0.78125 c/step) | `Synth.h`:183-197 | ✅ **VERIFIED-exact (2026-07-17)** — mechanism from the binary; clone confirmed correct in *pattern* (outer/inner=3.0) and at max (36.33 vs 36.25 c) | direct disasm; supersedes the FFT guess and the "ENTANGLED / no isolable sub-osc loop" claim. Open: ~3% mid-knob scale (worst +0.97c @b≈224) + VAZ's 32-step quantisation |
| Osc3 audio-rate footage | `FUN_004dbddc`:~1310 ratio + LFO1 rateVal | mixer opt, `2^((b−144)/48)` | ⏸ **PARKED (anchor-consistent)** | 2 operand hypotheses survive; DSP-struct VMT not RE'd. **Mechanism now confirmed (2026-07-11): LFO1↔Osc3 is an explicit mixer-source mode-flag, NOT a rate threshold** — only mix-ch3 offers "Oscillator 3" (DFM `MixerSourcePopup`), audio already keys off `mix3Src==1`. GUI now relabels the LFO1 panel → "Oscillator 3" on that condition (`bindLfo1Identity`, matches Core.dll relabel @1109628/45). Only the *footage-pitch value* stays parked (VMT). **↳ RUNTIME DUMP BLOCKED (2026-07-14):** `FUN_004dbddc`:~1310 is the fixed-point filter/osc DSP; the footage ratio has **2 surviving operand hypotheses** and the object shares #8's blocker — the headless standalone can't isolate it (`dump_seq_runtime.py`). Clone keeps `2^((b−144)/48)`; **resolve via audio-render extraction** (render osc3 at each footage setting, measure the pitch → the exact mapping) — a focused follow-up, not a headless dump. **↳ ✅ RESOLVED 2026-07-15 — AUDIO-RENDER MEASURED (`tools/osc3_footage_probe.py` + `osc3_intermediate_check.py`):** rendered the REAL VAZ via **File→Capture** (the loopback path captures pure silence here — it peak-normalises on write, so silence *looks* like signal; must not be used) with Osc3 the sole audible source, base = a **VAZ-made `Osc 3.v2p`**, sweeping `lfo1rate`; f0 from the spectrum. Measured (C3=130.81): b=48→32, 96→66, **120→90.8**, 144→129.2, **168→183.2**, 192→261, **216→368**, 240→522 Hz — **8/8 within 3% of `C3·2^((b−144)/48)` (worst 2.1%)**, incl. non-octave points ⇒ the law is **continuous, not octave-stepped**. Both static survivors (identical as pitch fns) **CONFIRMED**; rate-curve `e^(0.036(b−144))` **REFUTED**. Sweeping `+0x94` 48..240 left pitch unchanged ⇒ **Osc3 pitch is driven by `lfo1rate`, not `+0x94`** → the clone's source is correct. **No DSP change needed.** Oracle `osc3_footage_pitch` = VERIFIED (measured). |
| 🐞→✅ **mixer-source enum OFF BY ONE — FOUND + FIXED 2026-07-15** | **VAZ uses ONE mixer-source popup, item 0 relabelled per channel** → all 3 ch share `0=own osc, 1=RingMod, 2=Noise, 3=Ext, 4/5=ModAmp`. Ground truth: Core.dll popup captions **@0x183812** (Oscillator 3 / Ring Modulator / Noise / External Input / Mod Amp 1 / Mod Amp 2) + a real VAZ Osc3-vs-RingMod preset byte-diff (`mix3src` @1432 = 0 vs 1) | ch3 listed **Noise first** (`0=Noise,1=Osc3`) + DSP gated on `mix3Src==1` → a real-VAZ Osc3 patch loaded as **"Noise"** and Osc3 never sounded. mix1/mix2 were already correct. | ✅ **FIXED + VERIFIED** | Fixed in `PluginProcessor.cpp` (list order + default idx 2=Noise), `SynthVoice.h`:283/287 (gate `==0`, c3 map `0=o3,1=rm,2=nz`), `ParameterIDs.hpp` comment, `index.html` (`<select>` order + `bindLfo1Identity` now keys index **0**). **Verified audibly:** clone renders the user's Osc3 patch at **30.00 Hz** vs real VAZ **30 Hz** (0.0% err, tonality 0.995 = pitched not noise); RingMod patch → 261.7 Hz ring-mod (correctly distinct). **Locked** by `VazV2PAudit` MIXER-SOURCE ENCODING LOCK (list order + the 2 real presets as fixtures in `tests/fixtures/`). Suites: Oracle 0 dev, Roundtrip 89/89, Audit 260 ok. |
