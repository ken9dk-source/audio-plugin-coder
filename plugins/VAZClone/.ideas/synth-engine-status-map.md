...
| Waveform generation (saw + pulse = algorithmic difference-of-saws) | `vaz_big.c`:176-320 branches | `Synth.h` osc switch | VERIFICERET (read) — **no numeric oracle diff yet** | address citation |
| Sample-osc interp (4-pt cubic) | `vaz_big.c`:979-991 | `Synth.h`:108 | VERIFICERET | line-by-line |
| Pulse oscillator | `vaz_big.c`:206-241 (BLEP) — transcribed, NOT wired | `Synth.h` case 1 (difference-of-saws) | ⚠ **BLEP PORT REVERTED (2026-07-11)** | Full BLEP engine was ported (`d473dad`, oracle `osc_pulse_blep` bit-exact vs transcription) **but reverted** — it sounded harsh/aliased ("C64 loading") in FL Studio vs real VAZ. **Methodology hole:** the oracle only proved clone == *my transcription*, never == *real VAZ*, so a mis-transcribed band-limiting passed. Headless couldn't reproduce the gross harshness (aliasing metric 0.0003-0.037, all <5%), only that the BLEP is measurably brighter (h17 +0.12 vs diff-of-saws). Current: difference-of-saws + `b/256` duty map + VAZ's freq-dependent edge clamp (audible, `dsp_audit pulse_default_audible`). Artifacts kept: `VAZPulseOsc.h`, `VAZPulseTables.h`, RPM-dumped `pulse_step_*` tables, oracle `osc_pulse_blep` (GAP row). **Re-port GATED on validating the transcription against a real VAZ render** (the "Sine (default sample)" GUI text is the sample-slot label, a red herring — pulse routes correctly to case 1). **Disassembly re-check (2026-07-11, `tools/disasm_pulse.py` + `check_inc_table.py`): the transcription is CONFIRMED FAITHFUL vs the actual machine code** — pulse recurrence `@0x4dc204-0x4dc2f7`, `iv8=((inc>>8)+(inc>>0xb)−0xe000)>>0xd` `@0x4dc220`, `pw=param[0xa4]>>1` `@0x4dc247`, min/max edge clamp `@0x4dc249`, step-table indexing + recombine all match instruction-for-instruction; **NO x87 floats**; increment `DAT_005445e0[pitch]=hz/sr·2^32` (clean 12-TET A440, confirmed vs RPM dump) = clone's `incU`. Table refs at `0x4dc27e/0x4dcb6e/0x4dd0ff` — B/C are the **saw** path (same math), not a different pulse. Render `0x4dbddc` is **vtable-dispatched** (no direct callers) → calling context (oversampling/unison) can't be traced statically. **Recurrence EXONERATED** — the FL harshness is NOT the pulse math; remaining candidates = a downstream/context step or an A/B confound, needing a real-VAZ render. |
