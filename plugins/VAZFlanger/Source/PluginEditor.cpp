...
VAZFlangerAudioProcessorEditor::VAZFlangerAudioProcessorEditor (VAZFlangerAudioProcessor& p)
    : AudioProcessorEditor (&p), audioProcessor (p)
{
    ...
    delayTimeAtt = attach (ParameterIDs::delay_time, delayTimeRelay);
    feedbackAtt  = attach (ParameterIDs::feedback,  feedbackRelay);
...
VAZFlangerAudioProcessorEditor::~VAZFlangerAudioProcessorEditor() {}

void VAZFlangerAudioProcessorEditor::paint (juce::Graphics& g) { g.fillAll (juce::Colour (0xff2a0d0d)); }

void VAZFlangerAudioProcessorEditor::resized()
{
    if (webView != nullptr) webView->setBounds (getLocalBounds());
}

int VAZFlangerAudioProcessorEditor::getControlParameterIndex (juce::Component&)
{
    return controlParamReceiver.getControlParameterIndex();
}

const char* VAZFlangerAudioProcessorEditor::getMimeForExtension (const juce::String& extension)
{
...
VAZFlangerAudioProcessorEditor::getResource (const juce::String& url)
{
